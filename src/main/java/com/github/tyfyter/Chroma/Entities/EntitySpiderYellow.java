...
{
    private final Predicate<Entity> attackEntitySelector = new Predicate<Entity>(){
        public boolean apply(Entity entityIn)
        {
            return entityIn instanceof EntityLivingBase && ((EntityLivingBase)entityIn).getHealth()<15 && ((EntityLivingBase)entityIn).getPositionVector().add(getPositionVector()).lengthVector()<=3;
        }
    };
        ...
        if (super.attackEntityAsMob(entityIn))
        {
            if (entityIn instanceof EntityLivingBase)
            {
        		...
        		if(!entityIn.noClip){
        		    ...
        		    ((EntityLivingBase)entityIn).addPotionEffect(new PotionEffect(Chroma.noClip.id, 2));
                }
            }
